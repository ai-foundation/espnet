...
from espnet2.tasks.tts_modified import TTSTask
...
class Text2Speech:
    ...
    def __init__(
        ...
        self.get_phonemes = TTSTask.build_g2p_fn(train_args)
    ...
    def __call__(
        ...
        if isinstance(text, str):
            out = self.preprocess_fn("<dummy>", dict(text=text), phonemes)
            text = out["text"]
        ...
        if None not in [pitch, energy, durations]:
        ...
        if decode_conf is not None:
            ...
            #if None not in [pitch, energy, durations]:
                #cfg.update(use_teacher_forcing=True)

        # inference
        if self.always_fix_seed:
            set_all_random_seed(self.seed)
        output_dict = self.tts.inference(**batch, **cfg)
        ...
        output_dict["phonemes"] = phonemes
        return output_dict
    
    
    @torch.no_grad()
    def get_phonemes_duration_pitch_energy(
        self,
        text: Union[str, torch.Tensor, np.ndarray],
        phonemes = None,
        spembs: Union[torch.Tensor, np.ndarray] = None,
        ...
        if self.always_fix_seed:
            set_all_random_seed(self.seed)
        output_dict = self.tts.get_duration_pitch_energy(**batch)
    ...
    def get_duration_pitch_energy(
        self,
        phonemes: Union[str, torch.Tensor, np.ndarray],
        spembs: Union[torch.Tensor, np.ndarray] = None,
        sids: Union[torch.Tensor, np.ndarray] = None,
    ) -> Dict[str, torch.Tensor]:
        
        return self.get_phonemes_duration_pitch_energy(text = 'dummy1',
        phonemes = phonemes, spembs = spembs, sids = sids,)
    ...
    def use_speech(self) -> bool:
        ...
        return getattr(self.tts, "use_gst", False) # self.use_teacher_forcing or getattr(self.tts, "use_gst", False)
